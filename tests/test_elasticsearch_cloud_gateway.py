...
from semantic_search.elastic import elastic_cloud
...
async def test_indexed_article_can_be_found_by_id():
    ...
    cloud_id = config['elastic_cloud']['cloud_id']
    user = config['elastic_cloud']['user']
    password = config['elastic_cloud']['password']
    async with elastic_cloud(cloud_id, user, password, index_name='articles-test') as elasticsearch_:
